...
data = pd.DataFrame(np.hstack(
    (datamat['emg'][:_size, :], datamat['rerepetition'][:_size, :], datamat['restimulus'][:_size, :])))
    ...
    if gesture == 0:
        for group in np.unique(regroup):
            ...
            for rep in np.unique(rerepetition):
                ...
                z = regroup[gr_i]
                w = rerepetition[gr_i]
                scio.savemat(file + '/rep-{:02d}_{:02d}.mat'.format(int(rep), int(z[0])),
        ...
        for rep in np.unique(rerepetition):
            ...
            z = regroup[gr_i]
            w = rerepetition[gr_i]
            scio.savemat(file + '/rep-{:02d}.mat'.format(int(rep)),
                         {'emg': x, 'stimulus': y, 'repetition': w, 'group': z})